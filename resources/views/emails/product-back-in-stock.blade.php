...
<head>
    <style>
        ...
        .title { font-size: 22px; font-weight: bold; margin-bottom: 10px; text-align: center; color: #4ade80; }
...
<body>
    <div class="container">
        ...
        <div class="title">Back in Stock!</div>
        <p>Good news! An item from your wishlist is back and ready for checkout.</p>
        <h3 style="color: #ffffff;">{{ $product->name }}</h3>
        <p class="price">${{ number_format($product->price, 2) }}</p>
        <p class="description">{{ $product->description }}</p>
        <div style="text-align: center; margin-top: 30px;">
            <a href="{{ url('/products/' . $product->slug) }}" class="button">Order Now</a>
        </div>
    </div>
</body>
</html>
